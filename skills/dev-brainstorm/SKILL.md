...
The goal is to produce a compact, concept-oriented brief that works well as upstream input for `dev-orchestrate` and `dev-investigate`.
...
- Make the note useful as a handoff or starting brief for `dev-orchestrate` or `dev-investigate`.
...
- `stop brainstorming`
- `pass this to dev-investigate`
- `use this as input to dev-orchestrate`
...
- `skills/dev-brainstorm/references/TEMPLATE.md`
...
- Do not invoke `dev-orchestrate` or any downstream subskill while brainstorming is still in progress.
...
- The final state should be a strong enough brief that `dev-orchestrate` or `dev-investigate` can start from it without being over-directed.
