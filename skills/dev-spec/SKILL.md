...
- Primary input:
  - An investigation report generated by `dev-investigate`, provided as pasted markdown or a file path
...
1. Normalize the input into:
   ...
   - `audience`
   - `open_questions`
2. If a `dev-investigate` report is provided, extract:
...
Create the destination path and template with `$HOME/.agents/skills/dev-spec/references/TEMPLATE.md` and save the filled draft to `$PWD/docs/specs/yyyy-MM-dd'T'HH-mm-ss'Z'_*.md`. Always use the `yyyy-MM-dd'T'HH-mm-ss'Z'_` UTC prefix for filenames. Do not include local paths or environment-specific information in the spec; use placeholders like `$PWD` instead.
