...
Create the destination path and template with: `$HOME/.agents/skills/dev-investigate/references/TEMPLATE.md` and save the filled report to `$PWD/docs/investigations/yyyy-MM-dd'T'HH-mm-ss'Z'_*.md`. Always use the `yyyy-MM-dd'T'HH-mm-ss'Z'_` UTC prefix for filenames. Do not include local paths or environment-specific information in the report; use placeholders like `$PWD` instead. 
