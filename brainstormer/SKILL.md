...
The goal is to produce a compact, concept-oriented brief that works well as upstream input for `supervisor` and `investigator`.
...
- Make the note useful as a handoff or starting brief for `supervisor` or `investigator`.
...
- `stop brainstorming`
- `pass this to investigator`
- `use this as input to supervisor`
...
- `brainstormer/references/TEMPLATE.md`
...
- Do not invoke `supervisor` or any downstream subskill while brainstorming is still in progress.
...
- The final state should be a strong enough brief that `supervisor` or `investigator` can start from it without being over-directed.
